...
func primeRange(a int, b int) {
	...
	for i := a; i <= b; i++ {
		p, err := isPrime(i)
		if err != nil {
			fmt.Println(err) // Continue with program logic
			os.Exit(0)
		}
		if p {
			fmt.Print(i, " ")
...
func main() {
	...
	for true {
		...
		if err != nil { //log.Fatal function prints the error message
			log.Fatal(err)
		}
		...
		case 1:
			...
			if err != nil {
				log.Fatal(err) //The program will exit immediately if there is an error
			}
			prime(in)
		case 2:
			var n1, n2 int
